...
1. **Decide whether there is anything to capture**
   ...
   The test: if an agent picked up this bundle cold in a week, would they need to be told
   this, and could they not get it any other way? A decision and the reasoning that ruled
   out alternatives, a precisely-named root cause, a non-obvious gotcha, a corrected
   standing belief, a measurement with its exact conditions, a worked-out procedure, a
   ruled-out approach, a mistake caused by this local setup or org's own conventions —
   these tend to qualify. What you did, which files you edited, what the user asked for,
   or a generic technology fact recoverable from the code or upstream docs at no cost —
   these do not. Read `worth-capturing.md` (in this skill's own directory) for the full
   criteria before making a close call.
...
3. **Read this bundle's content policy, if it has one**
   ...
   capturing, or add bundle-specific categories `worth-capturing.md` does not cover.
