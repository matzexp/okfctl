...
- [ ] 1.1 Rename `src/core/drafts.ts` to `src/core/dumps.ts`: `DEFAULT_DRAFTS_DIR` →
...
- [ ] 1.2 Update `src/commands/capture.ts` to import from `dumps.ts` and use the renamed
...
- [ ] 1.3 Update `src/cli.ts`: rename the global `--drafts-dir` option to `--dumps-dir`
      (used by `capture`); update the `draftsDir(command)` accessor to `dumpsDir(command)`.
- [ ] 1.4 Update `src/commands/init.ts` to scaffold `dumps/` instead of `drafts/`.
- [ ] 1.5 Rename `test/drafts.test.ts` to `test/dumps.test.ts` and update it for the
      renamed identifiers; update `test/capture.test.ts` and `test/init.test.ts` for the
      new default directory and flag name.
- [ ] 1.6 Update `src/core/agents/adapter.ts`'s reference to "the drafts area" (capture's
      guidance text) to say "the dumps area".

## 2. New drafts area for refined entries

- [ ] 2.1 Add a new `src/core/drafts.ts` (the old file having moved to `dumps.ts` in task
...
- [ ] 2.2 Wire `--drafts-dir` as a new global CLI option (now scoped to `refine`/`status`,
      not `capture`) in `src/cli.ts`, with a `draftsDir(command)` accessor.
- [ ] 2.3 `okfctl init` creates `drafts/` alongside `dumps/` when scaffolding a new bundle
      (`src/commands/init.ts`).
- [ ] 2.4 `test/drafts.test.ts` (new file, new meaning): mirror the old drafts tests
      (now in `dumps.test.ts`) for `resolveDraftsDir`, `inDrafts`, `draftConcepts`,
      including the escapes-the-bundle refusal.

## 3. `okfctl refine`

- [ ] 3.1 Add `src/commands/refine.ts`: `runRefine(sources, options)` resolving each
...
- [ ] 3.2 Write the target concept into the drafts area (or `--to` override) via
...
- [ ] 3.3 Build `sources[]` on the written concept: one entry per resolved source naming
...
- [ ] 3.4 Refuse to overwrite an existing target path; refuse a target outside the bundle;
      refuse a reserved filename (`index.md`, `log.md`) — matching `new`/`capture`/`move`.
- [ ] 3.5 Implement `--consume`: after a successful write, remove exactly the source files
...
- [ ] 3.6 Implement `--dry-run`: print the resolved target path, the frontmatter that
      would be written, and (when `--consume` is set) which source files would be
      removed — write nothing.
- [ ] 3.7 Log to the nearest `log.md`: the new concept, its type, its source ids, the
      actor, and whether sources were consumed.
- [ ] 3.8 Stage all writes/removals and roll back on partial failure, following the same
      staged-write/rollback pattern `runMove` already uses.
- [ ] 3.9 Wire the `refine` verb into `src/cli.ts` (source args, `--type`, `--title`,
...
- [ ] 4.1 Rename `Row.inDrafts` to `inDumps` in `src/commands/status.ts`, computed via
      `inDumps(concept.id, dumpsDir)`; add a new `inDrafts: boolean` computed via
      `inDrafts(concept.id, draftsDir)` for the new meaning.
- [ ] 4.2 Rename the `--drafts` CLI filter to `--dumps`; add a new `--drafts` filter for
      the new meaning. Both are included in `--all`'s unsegregated view; an entry is never
      counted in both inboxes.
- [ ] 4.3 Rename the existing `printInbox` output to the dumps inbox line; add a second,
      independent drafts inbox line, both shown by default when non-empty.
- [ ] 4.4 Rename `draftsDir`/`inDrafts` in `--json` output to `dumpsDir`/`inDumps`; add
      new `draftsDir`/`inDrafts` fields for the new meaning.

## 5. Tests

- [ ] 5.1 `test/refine.test.ts`: minimal invocation; missing `--type`/`--title`/`--by`
...
- [ ] 5.2 Extend `test/status-inbox.test.ts` for both inbox lines under their new names,
      `--dumps`/`--drafts` filtering, `--all` behavior, and the renamed/new JSON fields.
- [ ] 5.3 Extend `test/init.test.ts` to assert both `dumps/` and `drafts/` are scaffolded.
- [ ] 5.4 Update `test/hook.test.ts`, `test/move.test.ts`, and any other test referencing
      `drafts/` in its old meaning to use `dumps/` instead.
- [ ] 5.5 `npm test` passes end to end.

## 6. Skills

- [ ] 6.1 Edit `skills/okf-capture/SKILL.md`: every reference to "the drafts area" becomes
      "the dumps area"; `--drafts-dir` references become `--dumps-dir`.
- [ ] 6.2 Add `skills/okf-refine/SKILL.md`: establish bundle root; read the dumps inbox
...
- [ ] 6.3 Edit `skills/okf-review/SKILL.md` step 7: keep "Emptying the drafts inbox" as the
...
- [ ] 6.4 Edit `skills/okf-triage/SKILL.md`: report both inboxes under their (possibly
...
- [ ] 6.5 Add `commands/okf/refine.md`, matching the format of `commands/okf/capture.md`.
- [ ] 6.6 Update `commands/okf/capture.md`'s description ("into an OKF bundle's drafts
      area") to say "dumps area".

## 7. Docs and migration

- [ ] 7.1 Update `README.md` wherever it documents the drafts-area lifecycle: rename the
...
- [ ] 7.2 Update `docs/design.md` if it diagrams or narrates the current two-stage model.
