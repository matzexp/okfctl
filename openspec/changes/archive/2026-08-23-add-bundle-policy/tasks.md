...
- [x] 1.1 Add `src/core/policy.ts`: `POLICY_DIR = '.okf/policy'`, and three functions
...
- [x] 1.2 Write `contentPolicyTemplate()`'s content: restate `okf-capture`'s existing
...
- [x] 1.3 Write `sourcePolicyTemplate()`'s content: restate `okf-review`'s existing
...
- [x] 1.4 Write `fieldPolicyTemplate()`'s content: explain SPEC §11's baseline (`type`,
...
- [x] 1.5 Update `src/commands/init.ts`: add `.okf/policy/content-policy.md`,
...
- [x] 1.6 Confirm (via a test, not by inspection) that `.okf/` is excluded from
      `loadBundle`'s walk with no changes to `src/core/bundle.ts` — it already skips any
      dotfile-prefixed entry.

## 2. Tests

- [x] 2.1 `test/policy.test.ts`: `okfctl init` on an empty directory creates all three
...
- [x] 2.2 Extend `test/init.test.ts`'s "init scaffolds a conformant bundle" test to assert
...
- [x] 2.3 `npm test` passes end to end.

## 3. Skills read and apply policy

- [x] 3.1 Edit `skills/okf-capture/SKILL.md`: after "Establish the target bundle," add a
...
- [x] 3.2 Edit `skills/okf-refine/SKILL.md`: same pattern — read
...
- [x] 3.3 Edit `skills/okf-ingest/SKILL.md`: read `field-policy.md` when deciding
...
- [x] 3.4 Edit `skills/okf-review/SKILL.md`: read `source-policy.md` before step 3 ("check
      ...
      checking" guardrail.
- [x] 3.5 Update each edited skill's guardrails section with one line: policy narrows or
      extends judgment calls, never overrides an actor/provenance/citation guardrail.

## 4. Docs

- [x] 4.1 Update `README.md`: document `.okf/policy/` alongside "The dumps and drafts
...
- [x] 4.2 Update `docs/design.md`: add a short section on the policy directory design
...
- [x] 5.2 `npm test` passes end to end.
- [x] 5.3 Manually run `okfctl init` against a scratch directory and read the three
