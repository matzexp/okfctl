...
- [x] 1.1 Add `OutputFormat = 'table' | 'json' | 'yaml'` and
...
- [x] 1.2 Add a shared CLI option helper in `src/cli.ts` (mirroring the existing
...
- [x] 1.3 Refuse an unrecognized `--format` value with an error naming the accepted values,
      before any command-specific logic runs.
- [x] 1.4 Ensure every command's error/diagnostic output goes to `stderr`, never `stdout`,
...
- [x] 2.1 `src/commands/status.ts`: replace the inline `JSON.stringify` branch with
      `renderOutput`; keep the exact same data shape (no behavior change, only the
      rendering path).
- [x] 2.2 `src/commands/check.ts`: same swap.
- [x] 2.3 `src/commands/refs.ts`: same swap.
- [x] 2.4 `src/cli.ts`: add `--format <table|json|yaml>` alongside the existing `--json` on
      the `status`, `check`, and `refs` commands.
- [x] 2.5 Extend `test/commands.test.ts` (or the relevant per-command test files) to cover
...
- [x] 3.1 `src/core/search.ts`: compute each hit's area (dumps / drafts / corpus, via the
...
- [x] 3.2 Apply the trust-tier boost to each hit's score before ranking: `human-reviewed`
      ×1.5, `machine-confirmed` ×1.2, `unverified` ×1.0 — as constants next to the existing
      `BOOST` table, tunable independently of it.
- [x] 3.3 `src/commands/search.ts`: add area and trust tier as trailing columns in the
      existing table output.
- [x] 3.4 `src/commands/search.ts`: add `--format json`/`yaml` support via `renderOutput`,
...
- [x] 3.5 `src/cli.ts`: add `--format <table|json|yaml>` to the `search` command.
- [x] 3.6 Extend `test/search.test.ts`: area and trust tier appear correctly per hit; a
...
- [x] 4.1 Update `README.md`'s command reference for `search`'s new columns/flags and the
      shared `--format` flag wherever `--json` is currently documented.
- [x] 4.2 Update `docs/design.md` if it documents command output shapes.

## 5. Verification

- [x] 5.1 `npm test` passes end to end.
- [x] 5.2 Manually pipe `okfctl search <query> --format json` into `jq` against the test
