...
4. **Capture it**
   ...
   ```bash
   okfctl capture --title "<what was established>" --by "<your producer id>" --stdin
   ```
   ...
   `--dry-run` shows the resolved path and frontmatter first. Use it when the id matters.

5. **Report**

   Name the concept id and say it is a draft awaiting review. Do not promote it, do not
...
- Never invent a source. If the knowledge rests on something you did not check, say so in
  the body rather than citing it.
