...
export function contentPolicyTemplate(): string {
  return [
    ...
    '## Worth capturing',
    '',
    'The default bar, inherited from `okf-capture`. Durable knowledge outlives the',
    'session because the next reader cannot re-derive it from the repository, the',
    'ticket, or common sense:',
    '',
    '- **A decision and why**, when the "why" required weighing options a reader would',
    '  otherwise re-litigate.',
    '- **A root cause**, when finding it took real investigation — the actual mechanism,',
    '  named precisely.',
    '- **A non-obvious constraint or gotcha**, especially one that looks like it should',
    '  work and does not.',
    '- **A correction to a standing belief**, when something the team or the docs',
    '  assumed turns out to be false.',
    '- **A measurement**, recorded with its exact conditions, not just the headline',
    '  number.',
    '- **A reusable procedure**, when getting the order wrong is costly enough that the',
    '  next person should follow it rather than reinvent it.',
    '- **A negative result** — an approach tried and specifically ruled out.',
    '- **A mistake caused by this local setup or this org\'s own conventions** — the',
    '  highest-value category, because it is invisible to generic documentation and',
    '  training data. Nothing outside this bundle will ever record it.',
    '',
    '## Not worth capturing',
    '',
    'What you did, which files you edited, what the user asked for, or anything the',
    'repository already records at a glance. Generic technology facts recoverable from',
    'the code, the dependency, or upstream docs at no cost — a library\'s public API',
    'contract, a documented framework default, an error message that is the technology',
    'working as designed.',
    '',
